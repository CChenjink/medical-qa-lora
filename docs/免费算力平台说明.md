...
- **20k 数据实验**：使用 Google Colab（LoRA约1-1.5h，QLoRA约1.5-2h）
- **50k 数据实验**：使用 Kaggle Notebooks（LoRA约3-4h，QLoRA约4-5h）
- **100k 数据实验**：使用 Kaggle（LoRA约6-7h，QLoRA约8-9h）
- **QLoRA 实验**：显存需求更低（~8GB），训练时间约为LoRA的1.5倍
