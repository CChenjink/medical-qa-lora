# 实验说明文档

## 项目概述

- **基座模型**：Qwen2.5-3B-Instruct
- **任务**：中文医疗问答系统微调
- **核心实验**：研究训练数据规模对模型性能的影响
- **数据规模**：10k / 20k / 40k / 60k
- **微调方法**：LoRA / QLoRA

---

## 实验设计

本项目旨在研究训练数据规模对大模型微调效果的影响，设计了以下实验：

### 实验矩阵

| 实验编号 | 方法 | 数据量 | 配置文件 | 输出目录 | 预计时间 (T4) |
|---------|------|--------|----------|----------|---------------|
| EXP-01 | LoRA | 10k | lora_10k.yaml | outputs/lora_10k | ~0.5-1h |
| EXP-02 | LoRA | 20k | lora_20k.yaml | outputs/lora_20k | ~1-1.5h |
| EXP-03 | LoRA | 40k | lora_40k.yaml | outputs/lora_40k | ~2-3h |
| EXP-04 | LoRA | 60k | lora_60k.yaml | outputs/lora_60k | ~3-4.5h |
| EXP-05 | QLoRA | 10k | qlora_10k.yaml | outputs/qlora_10k | ~0.7-1.2h |
| EXP-06 | QLoRA | 20k | qlora_20k.yaml | outputs/qlora_20k | ~1.5-2h |
| EXP-07 | QLoRA | 40k | qlora_40k.yaml | outputs/qlora_40k | ~3-4h |
| EXP-08 | QLoRA | 60k | qlora_60k.yaml | outputs/qlora_60k | ~4.5-6h |

### 实验目标

1. **数据规模影响**：对比 20k、50k、100k 三种数据规模对模型性能的影响
2. **方法对比**：对比 LoRA 和 QLoRA 两种微调方法的效果差异
3. **效率分析**：分析训练时间、显存占用等资源消耗
4. **最优配置**：找出性价比最高的数据规模和微调方法

---

## 环境准备

### 1. 安装依赖

```bash
pip install -r requirements.txt
```

### 2. 下载模型

```bash
# 从 ModelScope 下载（国内推荐）
python scripts/download_model.py \
    --model_name Qwen/Qwen2.5-3B-Instruct \
    --source modelscope \
    --save_dir ./models

# 或从 Hugging Face 下载
python scripts/download_model.py \
    --model_name Qwen/Qwen2.5-3B-Instruct \
    --source huggingface \
    --save_dir ./models
```

### 3. 准备数据

```bash
# 下载数据集
python scripts/download_data.py

# 预处理数据
python scripts/preprocess_data.py

# 准备不同规模数据集（10k/20k/40k/60k）
python scripts/prepare_data_splits.py
```

---

## 实验执行

### LoRA 实验

```bash
# 实验1：LoRA 10k
python train.py --config configs/lora_10k.yaml
python evaluate.py \
    --model_path outputs/lora_10k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/lora_10k/eval_results.json

# 实验2：LoRA 20k
python train.py --config configs/lora_20k.yaml
python evaluate.py \
    --model_path outputs/lora_20k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/lora_20k/eval_results.json

# 实验3：LoRA 40k
python train.py --config configs/lora_40k.yaml
python evaluate.py \
    --model_path outputs/lora_40k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/lora_40k/eval_results.json

# 实验4：LoRA 60k
python train.py --config configs/lora_60k.yaml
python evaluate.py \
    --model_path outputs/lora_60k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/lora_60k/eval_results.json
```

### QLoRA 实验

```bash
# 实验5：QLoRA 10k
python train.py --config configs/qlora_10k.yaml
python evaluate.py \
    --model_path outputs/qlora_10k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/qlora_10k/eval_results.json

# 实验6：QLoRA 20k
python train.py --config configs/qlora_20k.yaml
python evaluate.py \
    --model_path outputs/qlora_20k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/qlora_20k/eval_results.json

# 实验7：QLoRA 40k
python train.py --config configs/qlora_40k.yaml
python evaluate.py \
    --model_path outputs/qlora_40k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/qlora_40k/eval_results.json

# 实验8：QLoRA 60k
python train.py --config configs/qlora_60k.yaml
python evaluate.py \
    --model_path outputs/qlora_60k/checkpoint-best \
    --base_model_path models/Qwen_Qwen2.5-3B-Instruct \
    --output_file outputs/qlora_60k/eval_results.json
```

---

## 结果汇总

### 汇总所有实验结果

```bash
python scripts/summarize_results.py
```

输出文件：
- `outputs/summary/results_summary.csv` - 所有实验结果汇总表
- `outputs/summary/data_scale_comparison.png` - 数据规模对比图
- `outputs/summary/method_comparison.png` - 方法对比图

### 评估指标

记录以下指标用于对比分析：

| 指标 | 说明 |
|------|------|
| ROUGE-1 | 单词级别的召回率 |
| ROUGE-2 | 双词级别的召回率 |
| ROUGE-L | 最长公共子序列 |
| BLEU | 机器翻译评估指标 |
| 训练时间 | 完整训练所需时间 |
| 显存占用 | 峰值显存使用量 |
| 推理速度 | 每秒处理的样本数 |

---

## 实验时间规划

### 第1周：环境搭建和小规模实验

- 环境准备和依赖安装
- 数据下载和预处理
- 运行 10k 数据实验（LoRA + QLoRA）
- 验证实验流程

### 第2周：中规模实验

- 运行 20k 数据实验（LoRA + QLoRA）
- 运行 40k 数据实验（LoRA + QLoRA）
- 对比不同数据规模的性能差异

### 第3周：大规模实验和结果汇总

- 运行 60k 数据实验（LoRA + QLoRA）
- 完成所有实验
- 汇总实验结果
- 初步分析数据规模影响

### 第4周：分析和报告

- 数据规模影响分析
- LoRA vs QLoRA 方法对比
- 绘制对比图表
- 撰写实验报告

---

## 资源需求

### 硬件要求

| 实验 | 显存需求 | 推荐GPU |
|------|---------|---------|
| LoRA 20k | ~12GB | T4 / V100 |
| LoRA 50k | ~12GB | T4 / V100 |
| LoRA 100k | ~12GB | T4 / V100 |
| QLoRA 20k | ~8GB | T4 |
| QLoRA 50k | ~8GB | T4 |
| QLoRA 100k | ~8GB | T4 |

### 免费算力平台

1. **Google Colab**
   - 免费 T4 GPU（15GB显存）
   - 每次12小时使用时间
   - 适合 20k 和 50k 实验

2. **Kaggle Notebooks**
   - 免费 P100 GPU（16GB显存）
   - 每周30小时使用时间
   - 适合所有实验

3. **AutoDL**
   - 新用户有免费额度
   - 按需付费
   - 适合 100k 大规模实验

详见：[免费算力平台说明.md](免费算力平台说明.md)

---

## 注意事项

### 实验记录

每个实验需要记录：
1. 训练开始和结束时间
2. 最终评估指标（ROUGE、BLEU等）
3. 显存占用峰值
4. 训练过程中的loss曲线
5. 遇到的问题和解决方案

### 数据备份

重要文件需要及时备份：
- 训练好的模型检查点
- 评估结果JSON文件
- 训练日志文件
- TensorBoard日志

### 常见问题

1. **显存不足**：使用QLoRA或减小batch size
2. **训练中断**：使用`--resume_from_checkpoint`恢复训练
3. **评估慢**：使用`--max_samples`限制评估样本数
4. **结果不理想**：检查数据质量和超参数设置

---

## 参考资源

- [项目主 README](../README.md) - 项目整体说明
- [使用指南](使用指南.md) - 详细操作步骤
- [配置文件说明](../configs/README.md) - 配置参数详解
- [免费算力平台说明](免费算力平台说明.md) - GPU平台使用指南

---

**开始实验**：按照上述步骤依次执行，遇到问题及时查看文档或寻求帮助 🚀
