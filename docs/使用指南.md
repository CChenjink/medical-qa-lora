...
```bash
python train.py \
    --config configs/lora_20k.yaml \
    --resume_from_checkpoint outputs/lora_20k/checkpoint-1200
```
...
data_config:
  train_file: "./data/processed/train_20k.json"
...
```bash
# 使用 TensorBoard
tensorboard --logdir outputs/lora_20k/logs

# 或查看日志文件
cat outputs/lora_20k/logs/events.out.tfevents.*
```
